...
class DBService:
    ...
    def get_all_users(self):
        """Busca todos os usuários para o cache local."""
        try:
            with get_db_connection() as conn:
    ...
    def get_psicologas_list_for_signup(self):
        """Busca a lista de psicólogas no DB."""
        try:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT username FROM usuarios WHERE role = 'Psicóloga'")
                    psicologas = cur.fetchall()
                    # fetchall() retorna tuplas, ex: [('dra_ana',), ('dr_bruno',)]
                    # Precisamos convertê-las para uma lista de strings
    ...
    def get_pacientes_da_psicologa(self, psicologa_username: str):
        """Busca a lista de pacientes vinculados a uma psicóloga a partir do cache."""
        pacientes = []
        try:
            # Col 2 = role, Col 3 = psicologa_associada
            for row in self.all_users_data:
                if len(row) > 3 and row[2] == "Paciente" and row[3] == psicologa_username:
                    pacientes.append(row[0]) # Adiciona o username (Col 0)
    ...
    def check_user(self, username, password):
        """Verifica usuário/senha e retorna o 'role' e 'psicologa_associada'."""
        try:
            # (Inseguro, vamos adicionar hashing depois)
            for row in self.all_users_data:
    ...
    def create_user(self, username, password, psicologa_selecionada):
        """Cria um novo usuário 'Paciente' vinculado a uma psicóloga."""
        ...
        try:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    # Atualiza o cache local de usuários e psicólogas
            ...
            # A 'conn' é fechada automaticamente pelo 'with'
    ...
    def write_checkin(self, checkin: CheckinFinal, gemini_data: GeminiResponse, paciente_id: str, psicologa_id: str, compartilhado: bool):
        """Salva o check-in no banco de dados SQL."""
        try:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    # Garantindo 12 colunas, como definido no init_db.py
                    cur.execute(
                        """
    ...
    def get_all_checkin_data(self):
        """Busca todos os registros de check-in (para o histórico)."""
        try:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
    ...
    def get_recados_paciente(self, paciente_id: str):
        ...
        try:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        "SELECT * FROM recados WHERE paciente_id = %s ORDER BY timestamp DESC LIMIT 20",
                        (paciente_id,)
                    )
                    rows = cur.fetchall()
                    headers = [desc[0] for desc in cur.description]
            ...
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        (psicologa_id, paciente_id, mensagem)
                    )
                    conn.commit()
                    print(f"Recado de {psicologa_id} para {paciente_id} salvo no SQL.")
    ...
    def delete_last_record(self, paciente_id: str):
        ...
        try:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    # Encontra o ID do registro mais recente
                    cur.execute(
